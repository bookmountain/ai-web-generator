/**
 * Visual editor utility for managing in-iframe visual editing.
 */
...
export class VisualEditor {
  ...
  /** Initialize the editor with the preview iframe. */
  ...
  /** Toggle edit mode. Returns the new edit-mode state. */
  toggleEditMode() {
    if (this.isEditMode) {
  ...
  /** Sync state with the iframe and clear overlays when edit mode is off. */
  syncState() {
    if (!this.isEditMode) {
      this.sendMessageToIframe({
        type: "CLEAR_ALL_EFFECTS",
  ...
  /** Clear the current element selection in the iframe. */
  clearSelection() {
    this.sendMessageToIframe({
  ...
  /** Call when the iframe has finished loading. */
  onIframeLoad() {
    if (this.isEditMode) {
      setTimeout(() => {
        this.injectEditScript()
  ...
  private injectEditScript() {
    ...
    const waitForIframeLoad = () => {
      try {
        ...
        // Ignore injection failures (e.g. cross-origin iframe)
      }
    }
      ...
      (function() {
        ...
        window.addEventListener('message', (event) => {
         ...
         function showEditTip() {
           ...
           tip.id = 'edit-tip';
           tip.innerHTML = '🎯 Edit mode on<br/>Hover to inspect, click to select';
           tip.style.cssText = \`
             position: fixed;
             top: 20px;
             right: 20px;
